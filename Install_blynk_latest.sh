...
if [ "$(whoami)" != "root" ]; then
	echo "You have to run this script as Superuser! in order to install Install_blynk_latest"
	exit 1
fi

###	CHECKING IF THE FOLDER EXIST
if [ -d "/home/pi/blynk-libraryX" ]; then
	echo "install Install_blynk_latest is already installed......."
	exit 1
fi
...
if [ ! -d "/home/pi/blynk-libraryX" ]; then
	...
	apt-get update && sudo apt-get upgrade																	| tee -a 	"$LOG_FILE"
