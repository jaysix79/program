...
if [ "$(whoami)" != "root" ]; then
	echo "You have to run this script as Superuser! in order to install Install_blynk"
	exit 1
fi

###	CHECKING IF THE FOLDER EXIST
if [ -d "/home/pi/blynk-library" ]; then
	echo "install Install_blynk is already installed......."
	exit 1
fi
...
if [ ! -d "/home/pi/blynk-library" ]; then
	...
	cd /home/pi/blynk-library/linux 																		| tee -a 	"$LOG_FILE"
